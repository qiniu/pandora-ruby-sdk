...
class Pandora::Transport::Transport::HTTP::FaradayTest < Test::Unit::TestCase
  include Pandora::Transport::Transport::HTTP
  ...
  context "Faraday transport" do
    ...
    should "pass the selector_class options to collection" do
      @transport = Faraday.new :hosts => [ { :host => 'foobar', :port => 1234 } ],
                               :options => { :selector_class => Pandora::Transport::Transport::Connections::Selector::Random }
      assert_instance_of Pandora::Transport::Transport::Connections::Selector::Random,
                         @transport.connections.selector
    end

    should "pass the selector option to collection" do
      @transport = Faraday.new :hosts => [ { :host => 'foobar', :port => 1234 } ],
                               :options => { :selector => Pandora::Transport::Transport::Connections::Selector::Random.new }
      assert_instance_of Pandora::Transport::Transport::Connections::Selector::Random,
                         @transport.connections.selector
    end
