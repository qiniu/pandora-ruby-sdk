...
class Pandora::Transport::ClientTest < Test::Unit::TestCase
  ...
  context "Client" do
    setup do
      Pandora::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.stubs(:__build_connections)
      @client = Pandora::Transport::Client.new
    end

    should "have default transport" do
      assert_instance_of Pandora::Transport::Client::DEFAULT_TRANSPORT_CLASS, @client.transport
    end

    should "instantiate custom transport class" do
      client = Pandora::Transport::Client.new :transport_class => DummyTransport
      assert_instance_of DummyTransport, client.transport
    end

    should "take custom transport instance" do
      client = Pandora::Transport::Client.new :transport => DummyTransport.new
      assert_instance_of DummyTransport, client.transport
    end
    ...
    should "send GET request as POST with the send_get_body_as option" do
      ...
      client = Pandora::Transport::Client.new :transport => transport, :send_get_body_as => 'POST'
    ...
    context "when passed hosts" do
      should "have localhost by default" do
        c = Pandora::Transport::Client.new
        assert_equal 'localhost', c.transport.hosts.first[:host]
      end

      should "take :hosts, :host, :url or :urls" do
        c1 = Pandora::Transport::Client.new :hosts => ['foobar']
        c2 = Pandora::Transport::Client.new :host  => 'foobar'
        c3 = Pandora::Transport::Client.new :url   => 'foobar'
        c4 = Pandora::Transport::Client.new :urls  => 'foo,bar'
    ...
    context "when the URL is set in the environment variable" do
      setup    { ENV['PANDORA_URL'] = 'foobar' }
      teardown { ENV.delete('PANDORA_URL')     }
    ...
    context "detecting adapter for Faraday" do
      setup do
        Pandora::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.unstub(:__build_connections)
      ...
      should "use the adapter from arguments" do
        c = Pandora::Transport::Client.new :adapter => :typhoeus
        handlers = c.transport.connections.all.first.connection.builder.handlers
    ...
    context "configuring Faraday" do
      ...
      should "apply faraday adapter" do
        c = Pandora::Transport::Client.new do |faraday|
    ...
    context "when passed options" do
      ...
      should "transfer selected host parts from URL into the 'http' options" do
        c = Pandora::Transport::Client.new :url => 'https://U:P@node1:8080'
