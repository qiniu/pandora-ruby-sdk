...
class Pandora::Transport::Transport::Connections::ConnectionTest < Test::Unit::TestCase
  include Pandora::Transport::Transport::Connections
  ...
  context "Connection" do
    ...
    should "return full path" do
      c = Connection.new
      assert_equal 'v5/repos/test/search', c.full_path('v5/repos/test/search')
      assert_equal 'v5/repos/test/search', c.full_path('v5/repos/test/search', {})
      assert_equal 'v5/repos/test/search?foo=bar', c.full_path('v5/repos/test/search', {:foo => 'bar'})
      assert_equal 'v5/repos/test/search?foo=bar+bam', c.full_path('v5/repos/test/search', {:foo => 'bar bam'})
    end

    should "return full url" do
      c = Connection.new :host => { :protocol => 'http', :host => 'localhost', :port => '9188' }
      assert_equal 'http://localhost:9188/v5/repos/test/search?foo=bar', c.full_url('v5/repos/test/search', {:foo => 'bar'})
    end

    should "return full url with credentials" do
      c = Connection.new :host => { :protocol => 'http', :user => 'U', :password => 'P', :host => 'localhost', :port => '9188' }
      assert_equal 'http://U:P@localhost:9188/v5/repos/test/search?foo=bar', c.full_url('v5/repos/test/search', {:foo => 'bar'})
    end

    should "return full url with path" do
      c = Connection.new :host => { :protocol => 'http', :host => 'localhost', :port => '9188', :path => '/foo' }
      assert_equal 'http://localhost:9188/foo/v5/repos/test/search?foo=bar', c.full_url('v5/repos/test/search', {:foo => 'bar'})
    end
