...
module Pipeline
  ...
  module API
    module Utils
      ...
      # Parses the Pipeline source code and returns a Hash of REST API information/specs.
      #
      # Example:
      #
      #     {
      ...
      def __parse_java_source(path)
        ...
        prefix = "src/main/java/org/pipeline/rest/action"
    ...
    # Contains a generator which will parse the Pipeline *.java source files,
    ...
    #             --verbose \
    #             --crawl \
    #             --pipeline=/path/to/pipeline/source/code
    #
    # Features:
    #
    ...
    # * Search the <http://pipeline.org> website to get proper documentation URLs
    ...
    class JsonGenerator < Thor
      ...
      #
      desc "generate", "Generate JSON API spec files from Pipeline source code"
      ...
      method_option :pipeline, default: __root.join('tmp/pipeline'), desc: 'Path to directory with Pipeline source code'
      method_option :crawl,     type: :boolean, default: false,            desc: 'Extract URLs from Pipeline website'
      ...
      def generate
        ...
        rest_actions = Utils.__parse_java_source(options[:pipeline].to_s)

        if rest_actions.empty?
          say_status 'ERROR', 'Cannot find Pipeline source in ' + options[:pipeline].to_s, :red
          exit(1)
        end
        ...
        rest_actions.each do |name, info|
          ...
          if options[:crawl]
            begin
              response = RestClient.get "http://search.pipeline.org/pipeline-search-website/guide/_search?q=#{URI.escape(name.gsub(/\./, ' '))}"
              hits = JSON.load(response)['hits']['hits']
              if hit = hits.first
                if hit['_score'] > 0.2
                  doc_title = hit['fields']['title']
                  doc_url   = "http://pipeline.org" + hit['fields']['url']
                end
              end
            rescue Exception => e
