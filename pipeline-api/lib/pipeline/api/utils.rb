module Pipeline
  module API
    ...
    module Utils
      ...
      #   Pipeline::API.settings[:skip_parameter_validation] = true
      ...
      def __validate_and_extract_params(arguments, params=[], options={})
        if options[:skip_parameter_validation] || Pipeline::API.settings[:skip_parameter_validation]
          arguments
        else
      ...
      def __report_unsupported_parameters(arguments, params=[])
        ...
        unsupported_params.each do |param|
          ...
          explanation = if param.is_a?(Hash)
            ...
            ". This parameter is not supported in the version you're using: #{Pipeline::API::VERSION}, and will be removed in the next release."
          end
      ...
      def __report_unsupported_method(name)
        ...
        message += ". This method is not supported in the version you're using: #{Pipeline::API::VERSION}, and will be removed in the next release."
	  ...
	  def __format_data(body)
		return "" unless body

        data = body.inject("") do |_data, rec|
          _rec = rec.inject("") do |r, (k, v)|
            f = ""
            if v.is_a? String
              v.each_char do |c|
                if c == '\r'
