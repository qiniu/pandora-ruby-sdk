...
module Logdb
  ...
  module API
    module Utils
      ...
      # Parses the Logdb source code and returns a Hash of REST API information/specs.
      #
      # Example:
      #
      #     {
      ...
      def __parse_java_source(path)
        ...
        prefix = "src/main/java/org/logdb/rest/action"
    ...
    # Contains a generator which will parse the Logdb *.java source files,
    ...
    #             --verbose \
    #             --crawl \
    #             --logdb=/path/to/logdb/source/code
    #
    # Features:
    #
    ...
    # * Search the <http://logdb.org> website to get proper documentation URLs
    ...
    class JsonGenerator < Thor
      ...
      #
      desc "generate", "Generate JSON API spec files from Logdb source code"
      ...
      method_option :logdb, default: __root.join('tmp/logdb'), desc: 'Path to directory with Logdb source code'
      method_option :crawl,     type: :boolean, default: false,            desc: 'Extract URLs from Logdb website'
      ...
      def generate
        ...
        rest_actions = Utils.__parse_java_source(options[:logdb].to_s)

        if rest_actions.empty?
          say_status 'ERROR', 'Cannot find Logdb source in ' + options[:logdb].to_s, :red
          exit(1)
        end
        ...
        rest_actions.each do |name, info|
          ...
          if options[:crawl]
            begin
              response = RestClient.get "http://search.logdb.org/logdb-search-website/guide/_search?q=#{URI.escape(name.gsub(/\./, ' '))}"
              hits = JSON.load(response)['hits']['hits']
              if hit = hits.first
                if hit['_score'] > 0.2
                  doc_title = hit['fields']['title']
                  doc_url   = "http://logdb.org" + hit['fields']['url']
                end
              end
            rescue Exception => e
